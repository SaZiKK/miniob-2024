 ...
 *
 * NOTE:
 * 数据库对象没有做完整的并发控制。比如在查找某张表的同时删除这个表，会引起访问冲突。这个控制是由使用者
...
class Db {
 public:
   ...
   */
  RC create_table(const char *table_name, span<const AttrInfoSqlNode> attributes, const StorageFormat storage_format = StorageFormat::ROW_FORMAT);
