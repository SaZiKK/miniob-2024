...
RC BplusTreeLogger::init_header_page(Frame *frame,
  ...
  return append_log_entry(
      make_unique<InitHeaderPageLogEntryHandler>(frame, header));
}

RC BplusTreeLogger::update_root_page(Frame *frame, PageNum root_page_num,
                                     PageNum old_page_num) {
  return append_log_entry(make_unique<UpdateRootPageLogEntryHandler>(
      frame, root_page_num, old_page_num));
}

RC BplusTreeLogger::leaf_init_empty(IndexNodeHandler &node_handler) {
  return append_log_entry(
      make_unique<LeafInitEmptyLogEntryHandler>(node_handler.frame()));
}

RC BplusTreeLogger::node_insert_items(IndexNodeHandler &node_handler, int index,
                                      span<const char> items, int item_num) {
  return append_log_entry(make_unique<NormalOperationLogEntryHandler>(
      node_handler.frame(), LogOperation::Type::NODE_INSERT, index, items,
      item_num));
}

RC BplusTreeLogger::node_remove_items(IndexNodeHandler &node_handler, int index,
                                      span<const char> items, int item_num) {
  return append_log_entry(make_unique<NormalOperationLogEntryHandler>(
      node_handler.frame(), LogOperation::Type::NODE_REMOVE, index, items,
      item_num));
}

RC BplusTreeLogger::leaf_set_next_page(IndexNodeHandler &node_handler,
                                       PageNum page_num, PageNum old_page_num) {
  return append_log_entry(make_unique<LeafSetNextPageLogEntryHandler>(
      node_handler.frame(), page_num, old_page_num));
}

RC BplusTreeLogger::internal_init_empty(IndexNodeHandler &node_handler) {
  return append_log_entry(
      make_unique<InternalInitEmptyLogEntryHandler>(node_handler.frame()));
}

RC BplusTreeLogger::internal_create_new_root(IndexNodeHandler &node_handler,
                                             PageNum first_page_num,
                                             span<const char> key,
                                             PageNum page_num) {
  return append_log_entry(make_unique<InternalCreateNewRootLogEntryHandler>(
      node_handler.frame(), first_page_num, key, page_num));
}

RC BplusTreeLogger::internal_update_key(IndexNodeHandler &node_handler,
                                        int index, span<const char> key,
                                        span<const char> old_key) {
  return append_log_entry(make_unique<InternalUpdateKeyLogEntryHandler>(
      node_handler.frame(), index, key, old_key));
}

RC BplusTreeLogger::set_parent_page(IndexNodeHandler &node_handler,
                                    PageNum page_num, PageNum old_page_num) {
  return append_log_entry(make_unique<SetParentPageLogEntryHandler>(
      node_handler.frame(), page_num, old_page_num));
}

RC BplusTreeLogger::append_log_entry(
    unique_ptr<bplus_tree::LogEntryHandler> entry) {
  if (!need_log_) {
    return RC::SUCCESS;
  }
...
RC BplusTreeLogger::commit() {
  ...
  RC rc = log_handler_.append(lsn, LogModule::Id::BPLUS_TREE,
...
RC BplusTreeLogger::rollback(BplusTreeMiniTransaction &mtr,
                             BplusTreeHandler &tree_handler) {
  need_log_ = false;

  for (auto iter = entries_.rbegin(), itend = entries_.rend(); iter != itend;
       ++iter) {
    auto &entry = *iter;
...
RC BplusTreeLogger::redo(BufferPoolManager &bpm, const LogEntry &entry) {
  ASSERT(entry.module().id() == LogModule::Id::BPLUS_TREE,
         "invalid log entry: %s", entry.to_string().c_str());
  ...
  if (OB_FAIL(rc) || buffer_pool == nullptr) {
    LOG_WARN("failed to get buffer pool. rc=%s, buffer_pool_id=%d", strrc(rc),
             buffer_pool_id);
    return rc;
  }
...
RC BplusTreeLogger::__redo(LSN lsn, BplusTreeMiniTransaction &mtr,
                           BplusTreeHandler &tree_handler,
                           Deserializer &redo_buffer) {
  need_log_ = false;
  ...
  while (redo_buffer.remain() > 0) {
    ...
    rc = LogEntryHandler::from_buffer(tree_handler.buffer_pool(), redo_buffer,
                                      entry);
    if (OB_FAIL(rc)) {
    ...
    if (frame != nullptr) {
      if (frame->lsn() >= lsn) {
        LOG_TRACE("no need to redo. frame=%p:%s, redo lsn=%ld", frame,
                  frame->to_string().c_str(), lsn);
        frame->unpin();
        continue;
      } else {
    ...
    if (OB_FAIL(rc)) {
      LOG_WARN("failed to redo log entry. rc=%s, lsn=%d, entry=%s", strrc(rc),
               lsn, entry->to_string().c_str());
      break;
    }
...
BplusTreeMiniTransaction::BplusTreeMiniTransaction(
    BplusTreeHandler &tree_handler, RC *operation_result /* =nullptr */)
...
RC BplusTreeLogReplayer::replay(const LogEntry &entry) {
  return BplusTreeLogger::redo(buffer_pool_manager_, entry);
}
