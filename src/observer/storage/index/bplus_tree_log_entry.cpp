...
RC LogEntryHandler::from_buffer(DiskBufferPool &buffer_pool, common::Deserializer &buffer, unique_ptr<LogEntryHandler> &handler) {
  auto frame_getter = [&buffer_pool](PageNum page_num, Frame *&frame) -> RC { return buffer_pool.get_this_page(page_num, &frame); };
...
SetParentPageLogEntryHandler::SetParentPageLogEntryHandler(Frame *frame, PageNum parent_page_num, PageNum old_parent_page_num)
    : NodeLogEntryHandler(LogOperation::Type::SET_PARENT_PAGE, frame), parent_page_num_(parent_page_num), old_parent_page_num_(old_parent_page_num) {}
...
NormalOperationLogEntryHandler::NormalOperationLogEntryHandler(Frame *frame, LogOperation operation, int index, span<const char> items, int item_num)
...
RC NormalOperationLogEntryHandler::serialize_body(Serializer &buffer) const {
  ...
  if ((ret = buffer.write_int32(index_)) < 0 || (ret = buffer.write_int32(item_num_) < 0) || (ret = buffer.write_int32(item_bytes) < 0) ||
...
RC NormalOperationLogEntryHandler::deserialize(Frame *frame, LogOperation operation, Deserializer &buffer, unique_ptr<LogEntryHandler> &handler) {
  ...
  if ((ret = buffer.read_int32(index)) < 0 || (ret = buffer.read_int32(item_num)) < 0 || (ret = buffer.read_int32(item_bytes)) < 0) {
    return RC::INTERNAL;
  }
...
LeafInitEmptyLogEntryHandler::LeafInitEmptyLogEntryHandler(Frame *frame) : NodeLogEntryHandler(LogOperation::Type::LEAF_INIT_EMPTY, frame) {}
...
LeafSetNextPageLogEntryHandler::LeafSetNextPageLogEntryHandler(Frame *frame, PageNum new_page_num, PageNum old_page_num)
    : NodeLogEntryHandler(LogOperation::Type::LEAF_SET_NEXT_PAGE, frame), new_page_num_(new_page_num), old_page_num_(old_page_num) {}
...
RC InternalCreateNewRootLogEntryHandler::deserialize(Frame *frame, Deserializer &buffer, unique_ptr<LogEntryHandler> &handler) {
  ...
  int32_t page_num = -1;
  int32_t key_size = -1;
  if ((ret = buffer.read_int32(first_page_num)) < 0 || (ret = buffer.read_int32(page_num)) < 0 || (ret = buffer.read_int32(key_size)) < 0) {
    return RC::INTERNAL;
  }
...
InternalUpdateKeyLogEntryHandler::InternalUpdateKeyLogEntryHandler(Frame *frame, int index, span<const char> key, span<const char> old_key)
...
UpdateRootPageLogEntryHandler::UpdateRootPageLogEntryHandler(Frame *frame, PageNum root_page_num, PageNum old_page_num)
    : LogEntryHandler(LogOperation::Type::UPDATE_ROOT_PAGE, frame), root_page_num_(root_page_num), old_page_num_(old_page_num) {}
