...
LogReplayer *MvccTrxKit::create_log_replayer(Db &db, LogHandler &log_handler) { return new MvccTrxLogReplayer(db, *this, log_handler); }

////////////////////////////////////////////////////////////////////////////////

MvccTrx::MvccTrx(MvccTrxKit &kit, LogHandler &log_handler) : trx_kit_(kit), log_handler_(log_handler) {}

MvccTrx::MvccTrx(MvccTrxKit &kit, LogHandler &log_handler, int32_t trx_id) : trx_kit_(kit), log_handler_(log_handler), trx_id_(trx_id) {
  started_ = true;
  recovering_ = true;
}
      ...
      case Operation::Type::INSERT: {
        ...
        if (recovering_) {
          ...
          if (OB_SUCC(rc)) {
            ...
            LOG_WARN("failed to get record while rollback. table=%s, rid=%s, rc=%s", table->name(), rid.to_string().c_str(), strrc(rc));
            return rc;
          }
        }
