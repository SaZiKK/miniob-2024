...
RC MvccTrxKit::init() {
  ...
  fields_ = vector<FieldMeta>{
      ...
      FieldMeta("__trx_xid_begin", AttrType::INTS, 0 /*attr_offset*/,
                4 /*attr_len*/, false /*visible*/, -1 /*field_id*/),
      FieldMeta("__trx_xid_end", AttrType::INTS, 0 /*attr_offset*/,
                4 /*attr_len*/, false /*visible*/, -2 /*field_id*/)};
...
RC MvccTrx::delete_record(Table *table, Record &record) {
  ...
  RC rc = table->visit_record(record.rid(),
                              [this, table, &delete_result,
                               &end_field](Record &inplace_record) -> bool {
                                RC rc = this->visit_record(
                                    table, inplace_record,
  ...
  if (OB_FAIL(delete_result)) {
    LOG_TRACE("record is not visible. rid=%s, rc=%s",
              record.rid().to_string().c_str(), strrc(delete_result));
...
RC MvccTrx::update_record(Table *table, Record &record, const char *attr_name,
                          Value *value) {
...
RC MvccTrx::visit_record(Table *table, Record &record, ReadWriteMode mode) {
  ...
  if (begin_xid > 0 && end_xid > 0) {
    if (trx_id_ >= begin_xid && trx_id_ <= end_xid) {
      ...
      LOG_TRACE("record invisible. trx id=%d, begin xid=%d, end xid=%d",
    ...
    if (-begin_xid == trx_id_) {
      ...
      LOG_TRACE(
          "record invisible. someone is updating this record right now. trx "
          "id=%d, begin xid=%d, end xid=%d",
          trx_id_, begin_xid, end_xid);
      rc = RC::RECORD_INVISIBLE;
    }
  } else if (end_xid < 0) {
    ...
    if (mode == ReadWriteMode::READ_ONLY) {
      ...
      if (-end_xid != trx_id_) {
        ...
        LOG_TRACE(
            "record invisible. self has deleted this record. trx id=%d, begin "
            "xid=%d, end xid=%d",
            trx_id_, begin_xid, end_xid);
        rc = RC::RECORD_INVISIBLE;
      }
      ...
      if (-end_xid != trx_id_) {
        LOG_TRACE(
            "concurrency conflit. someone is deleting this record right now. "
            "trx id=%d, begin xid=%d, end xid=%d",
            trx_id_, begin_xid, end_xid);
...
void MvccTrx::trx_fields(Table *table, Field &begin_xid_field,
  ...
  ASSERT(trx_fields.size() >= 2, "invalid trx fields number. %d",
         trx_fields.size());
      ...
      case Operation::Type::INSERT: {
        ...
        if (recovering_) {
          ...
          if (OB_SUCC(rc)) {
            ...
            LOG_WARN(
                "failed to get record while rollback. table=%s, rid=%s, rc=%s",
...
RC find_table(Db *db, const LogEntry &log_entry, Table *&table) {
  ...
  switch (MvccTrxLogOperation(trx_log_header->operation_type).type()) {
    ...
    case MvccTrxLogOperation::Type::DELETE_RECORD: {
      ...
      if (nullptr == table) {
        LOG_WARN("no such table to redo. log record=%s",
                 trx_log_record->to_string().c_str());
