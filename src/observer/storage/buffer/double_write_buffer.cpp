...
struct DoubleWritePage {
 public:
  ...
  DoubleWritePage(int32_t buffer_pool_id, PageNum page_num, int32_t page_index,
                  Page &page);
 ...
 public:
  ...
  bool valid =
      true;  /// 表示页面是否有效，在页面被删除时，需要同时标记磁盘上的值。
  Page page;

  static const int32_t SIZE;
};

DoubleWritePage::DoubleWritePage(int32_t buffer_pool_id, PageNum page_num,
                                 int32_t page_index, Page &_page)
...
RC DiskDoubleWriteBuffer::open_file(const char *filename) {
  if (file_desc_ >= 0) {
    LOG_ERROR("Double write buffer has already opened. file desc=%d",
              file_desc_);
  ...
  if (fd < 0) {
    LOG_ERROR("Failed to open or creat %s, due to %s.", filename,
              strerror(errno));
...
RC DiskDoubleWriteBuffer::add_page(DiskBufferPool *bp, PageNum page_num,
                                   Page &page) {
  ...
  if (iter != dblwr_pages_.end()) {
    ...
    LOG_TRACE(
        "[cache hit]add page into double write buffer. "
        "buffer_pool_id:%d,page_num:%d,lsn=%d, dwb size=%d",
  ...
  DoubleWritePage *dblwr_page =
      new DoubleWritePage(bp->id(), page_num, page_cnt, page);
  dblwr_pages_.insert(
      std::pair<DoubleWritePageKey, DoubleWritePage *>(key, dblwr_page));
  LOG_TRACE(
      "insert page into double write buffer. "
      "buffer_pool_id:%d,page_num:%d,lsn=%d, dwb size:%d",
      bp->id(), page_num, page.lsn, static_cast<int>(dblwr_pages_.size()));
  ...
  if (OB_FAIL(rc)) {
    LOG_WARN(
        "failed to write page into double write buffer. rc=%s "
        "buffer_pool_id:%d,page_num:%d,lsn=%d.",
  ...
  if (page_cnt + 1 > header_.page_cnt) {
    ...
    if (lseek(file_desc_, 0, SEEK_SET) == -1) {
      LOG_ERROR("Failed to add page header due to failed to seek %s.",
                strerror(errno));
...
RC DiskDoubleWriteBuffer::write_page_internal(DoubleWritePage *page) {
  ...
  int64_t offset =
      page_index * DoubleWritePage::SIZE + DoubleWriteBufferHeader::SIZE;
  if (lseek(file_desc_, offset, SEEK_SET) == -1) {
    LOG_ERROR("Failed to add page %lld of %d due to failed to seek %s.", offset,
              file_desc_, strerror(errno));
    return RC::IOERR_SEEK;
  }

  if (writen(file_desc_, page, DoubleWritePage::SIZE) != 0) {
    LOG_ERROR("Failed to add page %lld of %d due to %s.", offset, file_desc_,
              strerror(errno));
    return RC::IOERR_WRITE;
  }
...
RC DiskDoubleWriteBuffer::write_page(DoubleWritePage *dblwr_page) {
  ...
  if (!dblwr_page->valid) {
    LOG_TRACE(
        "double write buffer write page invalid. "
        "buffer_pool_id:%d,page_num:%d,lsn=%d",
        dblwr_page->key.buffer_pool_id, dblwr_page->key.page_num,
        dblwr_page->page.lsn);
    return RC::SUCCESS;
  }
  RC rc =
      bp_manager_.get_buffer_pool(dblwr_page->key.buffer_pool_id, disk_buffer);
  ASSERT(OB_SUCC(rc) && disk_buffer != nullptr,
         "failed to get disk buffer pool of %d",
         dblwr_page->key.buffer_pool_id);

  LOG_TRACE(
      "double write buffer write page. buffer_pool_id:%d,page_num:%d,lsn=%d",
      dblwr_page->key.buffer_pool_id, dblwr_page->key.page_num,
      dblwr_page->page.lsn);
...
RC DiskDoubleWriteBuffer::read_page(DiskBufferPool *bp, PageNum page_num,
                                    Page &page) {
  scoped_lock lock_guard(lock_);
  DoubleWritePageKey key{bp->id(), page_num};
  auto iter = dblwr_pages_.find(key);
  if (iter != dblwr_pages_.end()) {
    ...
    LOG_TRACE(
        "double write buffer read page success. bp id=%d, page_num:%d, lsn:%d",
        bp->id(), page_num, page.lsn);
    return RC::SUCCESS;
  }
...
RC DiskDoubleWriteBuffer::clear_pages(DiskBufferPool *buffer_pool) {
  ...
  auto remove_pred =
      [&spec_pages,
       buffer_pool](const pair<DoubleWritePageKey, DoubleWritePage *> &pair) {
  ...
  sort(spec_pages.begin(), spec_pages.end(),
       [](DoubleWritePage *a, DoubleWritePage *b) {
  ...
  for_each(spec_pages.begin(), spec_pages.end(),
           [](DoubleWritePage *dbl_page) { delete dbl_page; });
...
RC DiskDoubleWriteBuffer::load_pages() {
  ...
  if (!dblwr_pages_.empty()) {
    LOG_ERROR(
        "Failed to load pages, due to double write buffer is not empty. "
        "opened?");
    return RC::BUFFERPOOL_OPEN;
  }

  if (lseek(file_desc_, 0, SEEK_SET) == -1) {
    LOG_ERROR("Failed to load page header, due to failed to lseek:%s.",
              strerror(errno));
    return RC::IOERR_SEEK;
  }
  ...
  if (ret != 0 && ret != -1) {
    LOG_ERROR(
        "Failed to load page header, file_desc:%d, due to failed to read "
        "data:%s, ret=%d",
  ...
  for (int page_num = 0; page_num < header_.page_cnt; page_num++) {
    int64_t offset = ((int64_t)page_num) * DoubleWritePage::SIZE +
                     DoubleWriteBufferHeader::SIZE;

    if (lseek(file_desc_, offset, SEEK_SET) == -1) {
      LOG_ERROR(
          "Failed to load page %d, offset=%ld, due to failed to lseek:%s.",
    ...
    if (ret != 0) {
      LOG_ERROR(
          "Failed to load page, file_desc:%d, page num:%d, due to failed to "
    ...
    if (check_sum == page.check_sum) {
      ...
      dblwr_pages_.insert(pair<DoubleWritePageKey, DoubleWritePage *>(
          key, dblwr_page.release()));
    } else {
      LOG_TRACE("got a page with an invalid checksum. on disk:%d, in memory:%d",
                page.check_sum, check_sum);
    }
  }

  LOG_INFO("double write buffer load pages done. page num=%d",
           dblwr_pages_.size());
  return RC::SUCCESS;
}
...
////////////////////////////////////////////////////////////////
RC VacuousDoubleWriteBuffer::add_page(DiskBufferPool *bp, PageNum page_num,
                                      Page &page) {
