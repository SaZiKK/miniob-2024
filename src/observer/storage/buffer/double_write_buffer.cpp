...
DiskDoubleWriteBuffer::DiskDoubleWriteBuffer(BufferPoolManager &bp_manager, int max_pages /*=16*/) : max_pages_(max_pages), bp_manager_(bp_manager) {}
...
RC DiskDoubleWriteBuffer::write_page(DoubleWritePage *dblwr_page) {
  ...
  LOG_TRACE("double write buffer write page. buffer_pool_id:%d,page_num:%d,lsn=%d", dblwr_page->key.buffer_pool_id, dblwr_page->key.page_num,
            dblwr_page->page.lsn);
...
RC DiskDoubleWriteBuffer::clear_pages(DiskBufferPool *buffer_pool) {
  ...
  LOG_INFO("clear pages in double write buffer. file name=%s, page count=%d", buffer_pool->filename(), spec_pages.size());

  // 页面从小到大排序，防止出现小页面还没有写入，而页面编号更大的seek失败的情况
  sort(spec_pages.begin(), spec_pages.end(), [](DoubleWritePage *a, DoubleWritePage *b) { return a->key.page_num < b->key.page_num; });

  RC rc = RC::SUCCESS;
  for (DoubleWritePage *dbl_page : spec_pages) {
    rc = buffer_pool->write_page(dbl_page->key.page_num, dbl_page->page);
    if (OB_FAIL(rc)) {
      LOG_WARN("Failed to write page %s:%d to disk buffer pool. rc=%s", buffer_pool->filename(), dbl_page->key.page_num, strrc(rc));
      break;
    }
...
////////////////////////////////////////////////////////////////
RC VacuousDoubleWriteBuffer::add_page(DiskBufferPool *bp, PageNum page_num, Page &page) { return bp->write_page(page_num, page); }
