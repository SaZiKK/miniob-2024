...
RC LogFileReader::open(const char *filename) {
  ...
  if (fd_ < 0) {
    LOG_WARN("open file failed. filename=%s, error=%s", filename,
             strerror(errno));
    return RC::FILE_OPEN;
  }
...
RC LogFileReader::iterate(function<RC(LogEntry &)> callback,
                          LSN start_lsn /*=0*/) {
  ...
  while (true) {
    ...
    if (0 != ret) {
      ...
      LOG_WARN("read file failed. filename=%s, ret = %d, error=%s",
               filename_.c_str(), ret, strerror(errno));
      return RC::IOERR_READ;
    }

    if (header.size < 0 || header.size > LogEntry::max_payload_size()) {
      LOG_WARN("invalid log entry size. filename=%s, size=%d",
               filename_.c_str(), header.size);
      return RC::IOERR_READ;
    }
    ...
    if (0 != ret) {
      LOG_WARN("read file failed. filename=%s, size=%d, ret=%d, error=%s",
               filename_.c_str(), header.size, ret, strerror(errno));
      return RC::IOERR_READ;
    }
    ...
    rc = callback(entry);
    if (OB_FAIL(rc)) {
      LOG_INFO("iterate log entry failed. entry=%s, rc=%s",
               entry.to_string().c_str(), strrc(rc));
      return rc;
    }
    LOG_TRACE("redo log iterate entry success. entry=%s",
...
RC LogFileReader::skip_to(LSN start_lsn) {
  ...
  if (off_t(-1) == pos) {
    LOG_WARN("seek file failed. seek to the beginning. filename=%s, error=%s",
  ...
  while (true) {
    ...
    if (header.lsn >= start_lsn) {
      ...
      if (off_t(-1) == pos) {
        LOG_WARN(
            "seek file failed. skip back log header. filename=%s, error=%s",
            filename_.c_str(), strerror(errno));
        return RC::IOERR_SEEK;
      }
      break;
    ...
    if (off_t(-1) == pos) {
      LOG_WARN(
          "seek file failed. skip log entry payload. filename=%s, error=%s",
          filename_.c_str(), strerror(errno));
      return RC::IOERR_SEEK;
    }
  }
...
RC LogFileWriter::write(LogEntry &entry) {
  ...
  if (entry.lsn() <= last_lsn_) {
    LOG_WARN(
        "write log entry failed. lsn is too small. filename=%s, last_lsn=%ld, "
        "entry=%s",
  ...
  /// 日志只写成功一部分到文件中非常难处理
  int ret = writen(fd_, reinterpret_cast<const char *>(&entry.header()),
                   LogHeader::SIZE);
  if (0 != ret) {
    LOG_WARN(
        "write log entry header failed. filename=%s, ret = %d, error=%s, "
        "entry=%s",
  ...
  if (0 != ret) {
    LOG_WARN(
        "write log entry payload failed. filename=%s, ret = %d, error=%s, "
        "entry=%s",
        filename_.c_str(), ret, strerror(errno), entry.to_string().c_str());
    return RC::IOERR_WRITE;
  }
  ...
  last_lsn_ = entry.lsn();
  LOG_TRACE("write log entry success. filename=%s, entry=%s", filename_.c_str(),
            entry.to_string().c_str());
  return RC::SUCCESS;
}
...
RC LogFileManager::init(const char *directory, int max_entry_number_per_file) {
  ...
  if (!filesystem::is_directory(directory_)) {
    ...
    if (!ret) {
      LOG_WARN("create directory failed. directory=%s, error=%s",
  ...
  for (const filesystem::directory_entry &dir_entry :
       filesystem::directory_iterator(directory_)) {
    ...
    if (log_files_.find(lsn) != log_files_.end()) {
      LOG_TRACE("duplicate log file. filename1=%s, filename2=%s",
                filename.c_str(),
                log_files_.find(lsn)->second.filename().c_str());
      continue;
    }
...
RC LogFileManager::get_lsn_from_filename(const string &filename, LSN &lsn) {
  if (!filename.starts_with(file_prefix_) ||
      !filename.ends_with(file_suffix_)) {
    return RC::INVALID_ARGUMENT;
  }

  string_view lsn_str(
      filename.data() + strlen(file_prefix_),
      filename.length() - strlen(file_suffix_) - strlen(file_prefix_));
  from_chars_result result =
      from_chars(lsn_str.data(), lsn_str.data() + lsn_str.size(), lsn);
  if (result.ec != errc()) {
...
RC LogFileManager::last_file(LogFileWriter &file_writer) {
  ...
  return file_writer.open(
      last_file_item->second.c_str(),
...
RC LogFileManager::next_file(LogFileWriter &file_writer) {
  ...
  return file_writer.open(file_path.c_str(),
                          lsn + max_entry_number_per_file_ - 1);
}
