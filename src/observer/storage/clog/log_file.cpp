...
RC LogFileManager::init(const char *directory, int max_entry_number_per_file) {
  ...
  for (const filesystem::directory_entry &dir_entry : filesystem::directory_iterator(directory_)) {
    ...
    if (log_files_.find(lsn) != log_files_.end()) {
      LOG_TRACE("duplicate log file. filename1=%s, filename2=%s", filename.c_str(), log_files_.find(lsn)->second.filename().c_str());
      continue;
    }
  ...
  LOG_INFO("init log file manager success. directory=%s, log files=%d", directory_.c_str(), static_cast<int>(log_files_.size()));
  return RC::SUCCESS;
}
...
RC LogFileManager::get_lsn_from_filename(const string &filename, LSN &lsn) {
  ...
  if (result.ec != errc()) {
    LOG_TRACE("invalid log file name: cannot calc lsn. filename=%s, error=%s", filename.c_str(), strerror(static_cast<int>(result.ec)));
    return RC::INVALID_ARGUMENT;
  }
