 ...
 * @details
 * 当libevent检测到某个连接上有消息到达时，比如客户端发消息过来、客户端断开连接等，就会
 * 调用这个回调函数。
  ...
  */
  ...
  auto sql_handler = [this, ag]() {
    ...
    if (RC::SUCCESS != rc) {
      ...
      // 或者就需要加锁处理并发问题
      // LOG_TRACE("add event. fd=%d, communicator=%p", event_get_fd(ag->ev),
      // this);
    }
  };
...
RC JavaThreadPoolThreadHandler::new_connection(Communicator *communicator) {
  ...
  /// EV_ET
  /// 表示边缘触发，有消息时只会触发一次，不会重复触发。这个标识在Linux平台上是支持的，但是有些平台不支持。
  /// 使用EV_ET边缘触发时需要注意一个问题，就是每次一定要把客户端发来的消息都读取完，直到read返回EAGAIN为止。
  /// 我们这里不使用边缘触发。
  /// 注意这里没有加
  /// EV_PERSIST，表示事件触发后会自动从event_base中删除，需要自己再手动加上这个标识。这是有必
