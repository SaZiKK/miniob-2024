...
RC ExpressionBinder::bind_unbound_field_expression(unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  if (unbound_field_expr->has_field_alias())
    context_.add_fields_alias_and_name(std::make_pair(field_alias, field_name));
  ...
  else
  {
    ...
    // 设置别名信息
    if (unbound_field_expr->has_field_alias())
      field.set_field_alias(unbound_field_expr->field_alias());
    if (unbound_field_expr->has_table_alias())
      field.set_table_alias(unbound_field_expr->table_alias());
