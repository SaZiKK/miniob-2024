                                 ...
                                 const std::vector<std::vector<Value>> right_tuple_list) const
{
   ...
   * TUPLES_TUPLES
   */
  if ((left.get_null() || right.get_null()) && (comp_ != CompOp::XXX_IS_NULL && comp_ != CompOp::XXX_IS_NOT_NULL))
    return RC::SUCCESS;
...
RC ComparisonExpr::check_value() const
{
   ...
   * TUPLES_TUPLES
   */
  if ((left_value.get_null() || right_value.get_null()) && (comp_ != CompOp::XXX_IS_NULL && comp_ != CompOp::XXX_IS_NOT_NULL))
    return RC::SUCCESS;
  if (this->comp_type() == CompType::LIST_LIST || this->comp_type() == CompType::TUPLES_LIST || this->comp_type() == CompType::LIST_TUPLES ||
      this->comp_type() == CompType::TUPLES_TUPLES)
  {
    ...
    : type_(type), child_(std::move(child))
{
  ...
  Value round_value;
  if (target_num != nullptr)
    rc = target_num->try_get_value(round_value);
  if (rc != RC::SUCCESS)
    round_value = Value((float)-1);

  rc = RC::INVALID_ARGUMENT;
  Value format_value;
  if (target_format != nullptr)
    rc = target_format->try_get_value(format_value);
  if (rc != RC::SUCCESS)
    format_value = Value("");
