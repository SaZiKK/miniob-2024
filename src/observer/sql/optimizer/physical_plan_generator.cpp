...
RC PhysicalPlanGenerator::create_vec(LogicalOperator &logical_operator,
                                     unique_ptr<PhysicalOperator> &oper) {
  RC rc = RC::SUCCESS;
...
RC PhysicalPlanGenerator::create_plan(TableGetLogicalOperator &table_get_oper,
  ...
  if (index != nullptr) {
    ...
    IndexScanPhysicalOperator *index_scan_oper = new IndexScanPhysicalOperator(
        table, index, table_get_oper.read_write_mode(), &value,
...
RC PhysicalPlanGenerator::create_plan(ProjectLogicalOperator &project_oper,
  ...
  auto project_operator = make_unique<ProjectPhysicalOperator>(
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper,
  ...
  oper = unique_ptr<PhysicalOperator>(
      new UpdatePhysicalOperator(table, field_meta, value));
...
RC PhysicalPlanGenerator::create_plan(GroupByLogicalOperator &logical_oper,
  ...
  ASSERT(logical_oper.children().size() == 1,
         "group by operator should have 1 child");
  ...
  if (OB_FAIL(rc)) {
    LOG_WARN(
        "failed to create child physical operator of group by operator. rc=%s",
        strrc(rc));
    return rc;
  }
...
RC PhysicalPlanGenerator::create_vec_plan(GroupByLogicalOperator &logical_oper,
                                          unique_ptr<PhysicalOperator> &oper) {
  RC rc = RC::SUCCESS;
...
RC PhysicalPlanGenerator::create_vec_plan(ProjectLogicalOperator &project_oper,
                                          unique_ptr<PhysicalOperator> &oper) {
  vector<unique_ptr<LogicalOperator>> &child_opers = project_oper.children();
...
RC PhysicalPlanGenerator::create_vec_plan(ExplainLogicalOperator &explain_oper,
                                          unique_ptr<PhysicalOperator> &oper) {
  vector<unique_ptr<LogicalOperator>> &child_opers = explain_oper.children();
