  ...
  */
  ...
  case ExprType::VALUELIST:
  {
    ...
    for (auto it : value_list)
      if (it.attr_type() == AttrType::DATE && !DateType::check_date(&it))
        return RC::INVALID_ARGUMENT;
  }
  break;
                                  ...
                                  FilterUnit *&filter_unit)
{
  ...
  if (condition.left_is_sub_query)
    left_expr = make_unique<SubQueryExpr>(condition.left_sub_query_stmt);
  if (condition.right_is_sub_query)
    right_expr = make_unique<SubQueryExpr>(condition.right_sub_query_stmt);
